...
def okta_mfa_login(password_login_response):
    ...
    # get the mfa challenge type to use
    challenge = soup.select_one("#factor_selector option[selected]")
    challengetype = challenge["value"]
    app = challenge.get_text("", strip=True).replace("Verify with ", "")
    # build idpmfaformsubmiturl where the login info will be POSTed
    form = soup.select_one("#{}-challenge-section form".format(challengetype))
    action = form["action"]
    idpmfaformsubmiturl = "https://{}{}".format(
                           urlparse(password_login_response.url).netloc
                           ,action)
    # Send SMS passcode if required
    if (challengetype == "SMS_FACTOR"):
        send_sms_passcode(password_login_response)
...
def okta_password_login(username,password,idp_entry_url):
    ...
    if "Sign in failed!" in response.text:
        ...
        sys.exit(1)
    response = okta_mfa_login(response)
    return response
